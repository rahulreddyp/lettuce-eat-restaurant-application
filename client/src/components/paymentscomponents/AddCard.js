...
const AddCard = () => {
    ...
    cardForm.userId = user.id
    cardForm.userEmail = user.email
    ...
    const handleSubmit = (event) => {
        ...
        if (Object.keys(currentErrors).length > 0) {
            ...
            cardForm.cvv = CryptoJS.AES.encrypt(JSON.stringify(cardForm.cvv), 'csci5409_project_group01').toString()
            saveCardDetails(cardForm)
                .then(response => {
                    if (response.success) {
                        setValidated(true)
                        setCardSuccess(true)
